...
export async function renderCardPage(root, params = {}) {
  ...
  root.innerHTML = `
    <style>
      ...
      .card-shell { background:var(--bg-elevated); border:1px solid var(--border-soft); border-radius:24px; padding:18px; }
      .card-header { display:flex; gap:18px; align-items:flex-start; }
      .cover { width:124px; min-width:124px; height:180px; border-radius:18px; background:var(--bg-soft); overflow:hidden; border:1px solid var(--border); }
      .cover img { width:100%; height:100%; object-fit:cover; }
      .cover-fallback { display:grid; place-items:center; height:100%; color:var(--text-soft); font-size:24px; font-weight:700; }
      .meta { flex:1; display:flex; flex-direction:column; gap:10px; min-width:0; }
      .title { font-size:24px; font-weight:800; line-height:1.22; color:var(--text); }
      .subtitle { font-size:14px; color:var(--text-soft); line-height:1.45; }
      .badges { display:flex; gap:8px; flex-wrap:wrap; }
      .badge { display:inline-block; padding:7px 11px; border-radius:999px; background:var(--accent-soft); font-size:12px; color:var(--text); }
      .actions { display:flex; gap:10px; margin-top:8px; flex-wrap:wrap; }
      .btn { padding:11px 16px; border-radius:14px; background:var(--surface); border:1px solid var(--border); font-weight:700; color:var(--text); }
      .btn.primary { background:var(--accent); color:#fff; border-color:transparent; }
      .btn.disabled { opacity:0.65; pointer-events:none; }
      .description-block { background:var(--surface); border:1px solid var(--border-soft); border-radius:18px; padding:16px; }
      .description-title { font-size:14px; font-weight:800; color:var(--text); margin-bottom:10px; }
      .description { line-height:1.75; color:var(--text-soft); white-space:pre-wrap; }
      .empty-description { color:var(--text-soft); }
      .card-status { min-height:20px; color:var(--text-soft); font-size:14px; }
