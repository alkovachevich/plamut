...
export function createPublicViewCore(deps){
  ...
  function renderPublicCardInternal(profile = {}){
    state.currentPublicProfile = profile;
  ...
  async function showPublicShareScreen(profile){
    state.isPublicView = true;
    renderPublicCardInternal(profile);
    hideAllScreens();
  ...
  function renderShareStateInternal(viewState = "loading"){
    ...
    document.getElementById("public-share-main-card")?.classList.toggle(
      "hidden",
      viewState === "loading" || viewState === "error" || !state.currentPublicProfile
    );
  ...
  function renderShareLibraryInternal(items = []){
    const grid = document.getElementById("public-share-preview-grid");
    if(!grid) return;

    state.currentPublicShareItems = Array.isArray(items)
      ? items.map((item) => ({ ...item, cover: item.cover || item.cover_url || "" }))
      : [];
  ...
  async function loadPublicShareRoute(token){
    ...
    hideAllScreens();
    document.getElementById("public-share-screen")?.classList.remove("hidden");
    renderShareStateInternal("loading");
    ...
    try {
      ...
      if(!profile || !isShareEnabled(profile)){
        state.currentPublicProfile = null;
        renderShareStateInternal("error");
        return true;
      }
      ...
      renderPublicCardInternal(state.currentPublicProfile);
      ...
      renderShareLibraryInternal(items);
  ...
  function openSharedLibrary(){
    if(document.body.classList.contains("public-route-active")){
      ...
      renderShareStateInternal(
        state.currentPublicShareItems.length
          ? "ready"
          : state.currentPublicShareState === "loading"
            ? "loading"
            : "empty"
    ...
    saveRouteState({
      screen: "category",
      category: getDefaultPublicCategory(),
      isPublicShareRoute: true,
      shareToken: state.activeShareToken || ""
    });
  ...
  return {
    ...
    syncPublicQrButtons,
    renderOwnerPanel,
    renderPublicCard: renderPublicCardInternal,
    showPublicShareScreen,
    renderShareState: renderShareStateInternal,
    renderShareLibrary: renderShareLibraryInternal,
