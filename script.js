...
// --- Stable navigation, auth and performance overrides ---
Object.assign(translations.en, {
  collections: {
    title: "Collections",
    subtitle: "Folders and grouped shelves across the library.",
    open: "Open"
  },
  settingsPage: {
    title: "Settings",
    subtitle: "Session, interface and sharing.",
    profile: "Profile",
    session: "Session",
    interface: "Interface",
    sharing: "Public library",
    currentPage: "Current page",
    restore: "Restored automatically after refresh"
  }
});
Object.assign(translations.ru, {
  collections: {
    title: "Коллекции",
    subtitle: "Папки и сгруппированные полки по всей библиотеке.",
    open: "Открыть"
  },
  settingsPage: {
    title: "Настройки",
    subtitle: "Сессия, интерфейс и публичная библиотека.",
    profile: "Профиль",
    session: "Сессия",
    interface: "Интерфейс",
    sharing: "Публичная библиотека",
    currentPage: "Текущая страница",
    restore: "Автовосстановление после обновления"
  }
});

let currentSupabaseSession = null;
let hasBootstrappedSupabaseSession = false;
let libraryHydrationPromise = null;
let hydratedLibraryUserId = "";
let hydratedLibraryAt = 0;
const LIBRARY_HYDRATION_TTL_MS = 1000 * 60 * 3;
const RELATION_BACKGROUND_BATCH = 3;

function persistSessionSnapshot(session){
  try {
    if(session?.user){
      localStorage.setItem("plamut_session_snapshot", JSON.stringify({
        userId: session.user.id,
        email: session.user.email || "",
        timestamp: new Date().toISOString()
      }));
    } else {
      localStorage.removeItem("plamut_session_snapshot");
    }
  } catch (_error) {
    // noop
  }
...
async function bootstrapSupabaseSession(force = false){
  if(hasBootstrappedSupabaseSession && !force){
  ...
  hasBootstrappedSupabaseSession = true;
  persistSessionSnapshot(currentSupabaseSession);
  return currentSupabaseSession;
}

getCurrentUser = async function getCurrentUserStable(){
  ...
  currentSupabaseSession = { ...(currentSupabaseSession || {}), user: data.user };
  persistSessionSnapshot(currentSupabaseSession);
  return data.user;
};

function invalidateLibraryHydration(){
  libraryHydrationPromise = null;
  hydratedLibraryUserId = "";
  hydratedLibraryAt = 0;
}

function normalizeSupabaseMediaRow(row = {}, category = ""){
  const mappedItem = {
    id: row.id,
    title: row.title,
    ...
    wikidata_entity_id: row.wikidata_entity_id || "",
    wikidata_label: row.wikidata_label || "",
    wikidata_description: row.wikidata_description || "",
    wikidata_last_enriched_at: row.wikidata_last_enriched_at || "",
    enrichment_status: row.enrichment_status || "idle",
    enrichment_confidence: Number(row.enrichment_confidence || 0),
  ...
  return { ...mappedItem, category };
}

async function hydrateLibraryFromSupabase(options = {}){
  const user = await getCurrentUser();
  if(!user){
  ...
  const isFresh = hydratedLibraryUserId === user.id && hydratedLibraryAt && (Date.now() - hydratedLibraryAt < LIBRARY_HYDRATION_TTL_MS);
  if(!options.force && isFresh && libraryHydrationPromise){
    return await libraryHydrationPromise;
  }

  libraryHydrationPromise = (async () => {
    const { data, error } = await supabaseClient
      .from("user_media")
      .select("*")
      .eq("user_id", user.id)
    ...
    if(error){
      console.error("Supabase bulk load error:", error);
      return false;
    }

    const cache = await getWikidataCache();
    const grouped = Object.fromEntries(Object.keys(demoData).map((category) => [category, []]));

    (data || []).forEach((row) => {
      const category = row.category || "";
      if(!grouped[category]) grouped[category] = [];
      const mapped = normalizeSupabaseMediaRow(row, category);
      mapped.enrichment = buildEnrichmentState({
        ...mapped.enrichment,
        ...(cache[getItemStorageKey(mapped)] || {})
      });
      const match = findBestMediaEntityMatch(grouped[category], mapped, { threshold: 0.88 });
      if(match){
        const index = grouped[category].findIndex((entry) => entry.id === match.id || getItemStorageKey(entry) === getItemStorageKey(match));
        grouped[category][index] = mergeMediaEntityRecords(grouped[category][index], mapped);
      } else {
    ...
    for(const category of Object.keys(demoData)){
      demoData[category] = (grouped[category] || []).map((entry) => ({ ...entry }));
      await applyFolderAssignmentsToItems(category);
      await applyTrackerOverridesToCategory(category);
    }

    hydratedLibraryUserId = user.id;
    hydratedLibraryAt = Date.now();
    markLibraryStateDirty();
    queueLibraryRelationSync();
    return true;
  })();
...
loadCategoryFromSupabase = async function loadCategoryFromSupabaseStable(category, options = {}){
  const loaded = await hydrateLibraryFromSupabase(options);
  if(!loaded){
    if(category && !demoData[category]) demoData[category] = [];
    renderShelf();
    return false;
  }
  if(category){
...
const baseSaveItemToSupabase = saveItemToSupabase;
saveItemToSupabase = async function saveItemToSupabaseStable(...args){
  const result = await baseSaveItemToSupabase(...args);
  if(result?.ok){
    invalidateLibraryHydration();
  }
  return result;
};

const baseDeleteItemFromSupabase = deleteItemFromSupabase;
deleteItemFromSupabase = async function deleteItemFromSupabaseStable(...args){
  const result = await baseDeleteItemFromSupabase(...args);
  if(result){
    invalidateLibraryHydration();
  }
  return result;
...
const baseUpdateStatusInSupabase = updateStatusInSupabase;
updateStatusInSupabase = async function updateStatusInSupabaseStable(...args){
  const result = await baseUpdateStatusInSupabase(...args);
  if(result){
    invalidateLibraryHydration();
  }
  return result;
...
const baseUpdateCategoryInSupabase = updateCategoryInSupabase;
updateCategoryInSupabase = async function updateCategoryInSupabaseStable(...args){
  const result = await baseUpdateCategoryInSupabase(...args);
  if(result){
    invalidateLibraryHydration();
  }
  return result;
...
const baseSaveItemFolderFromModal = saveItemFolderFromModal;
saveItemFolderFromModal = async function saveItemFolderFromModalStable(...args){
  const result = await baseSaveItemFolderFromModal(...args);
  invalidateLibraryHydration();
  return result;
};

async function itemNeedsBackgroundRelationSync(item){
  if(!item) return false;
  await ensureItemEnrichmentHydrated(item);
  const snapshot = await getCachedRelationSummarySnapshot(item);
  if(snapshot && (Number(snapshot.universeCount || 0) > 0 || Number(snapshot.confirmedCount || 0) > 0 || Number(snapshot.candidateCount || 0) > 0)){
    return false;
  }
  return shouldEnrichItem(item, false);
}

function queueLibraryRelationSync(items = getAllLibraryItems()){
  const runner = async () => {
    const queue = [];
    for(const item of items){
      if(queue.length >= RELATION_BACKGROUND_BATCH) break;
      if(await itemNeedsBackgroundRelationSync(item)){
        queue.push(item);
      }
    }
    triggerBackgroundWikidataEnrichment(queue, { limit: RELATION_BACKGROUND_BATCH, force: false });
  };
  if(window.requestIdleCallback){
    window.requestIdleCallback(() => runner().catch((error) => console.error("Idle relation sync error:", error)), { timeout: 800 });
  } else {
    window.setTimeout(() => runner().catch((error) => console.error("Deferred relation sync error:", error)), 120);
  }
}

const baseOpenCardByIdArchitecture = typeof productArchitectureOpenCardById === "function" ? productArchitectureOpenCardById : openCardById;
openCardById = async function openCardByIdStable(id){
  await baseOpenCardByIdArchitecture(id);
  const item = getItemById(currentCategory, id);
  if(!item) return;
  renderRelationSummaryLoadingState(item);
  const cachedSummary = await getCachedRelationSummarySnapshot(item);
  ...
  queueLibraryRelationSync([item]);
  await renderTrackerSummaryForCurrentItem();
  const deferredRender = () => renderUniverseDetailsForCurrentItem().catch((error) => console.error("Deferred relation render error:", error));
  if(window.requestIdleCallback){
    window.requestIdleCallback(deferredRender, { timeout: 400 });
  } else {
    window.setTimeout(deferredRender, 32);
  }
  persistAppRouteState();
};

const baseHideAllScreensStable = hideAllScreens;
hideAllScreens = function hideAllScreensStable(){
  baseHideAllScreensStable();
  ["collections-screen", "profile-screen", "settings-screen"].forEach((id) => document.getElementById(id)?.classList.add("hidden"));
};

getActiveProductView = function getActiveProductViewStable(){
  if(!document.getElementById("profile-screen")?.classList.contains("hidden")) return "profile";
  if(!document.getElementById("settings-screen")?.classList.contains("hidden")) return "settings";
  if(!document.getElementById("collections-screen")?.classList.contains("hidden")) return "collections";
  if(!document.getElementById("details-screen")?.classList.contains("hidden")) return "details";
  if(!document.getElementById("category-screen")?.classList.contains("hidden")) return "category";
  if(!document.getElementById("now-screen")?.classList.contains("hidden")) return "now";
  if(!document.getElementById("library-screen")?.classList.contains("hidden")) return "library";
...
buildAppRouteState = function buildAppRouteStateStable(){
  return {
    view: getActiveProductView(),
    category: currentCategory || "",
    itemId: currentOpenItemId || "",
...
buildAppRouteHash = function buildAppRouteHashStable(state = {}){
  const params = new URLSearchParams();
  if(state.filterStatus && state.filterStatus !== "All") params.set("status", state.filterStatus);
  if(state.filterFolder && state.filterFolder !== "All") params.set("folder", state.filterFolder);
  if(state.searchQuery) params.set("q", state.searchQuery);
  if(state.nowSort && state.nowSort !== "updated") params.set("sort", state.nowSort);
  if(state.scrollY) params.set("y", String(state.scrollY));
  const query = params.toString();
  if(state.view === "details" && state.category && state.itemId) return `#details/${encodeURIComponent(state.category)}/${encodeURIComponent(state.itemId)}${query ? `?${query}` : ""}`;
  if(state.view === "category" && state.category) return `#category/${encodeURIComponent(state.category)}${query ? `?${query}` : ""}`;
  if(state.view === "library") return `#library${query ? `?${query}` : ""}`;
  if(state.view === "collections") return `#collections${query ? `?${query}` : ""}`;
  if(state.view === "profile") return `#profile${query ? `?${query}` : ""}`;
  if(state.view === "settings") return `#settings${query ? `?${query}` : ""}`;
  if(state.view === "now") return `#now${query ? `?${query}` : ""}`;
  if(state.view === "stats") return `#stats${query ? `?${query}` : ""}`;
  if(state.view === "universes") return `#universes${query ? `?${query}` : ""}`;
  if(state.view === "auth") return "#auth";
...
parseAppRouteState = function parseAppRouteStateStable(){
  const rawHash = window.location.hash || "";
  if(rawHash.startsWith("#details/") || rawHash.startsWith("#category/") || rawHash.startsWith("#library") || rawHash.startsWith("#collections") || rawHash.startsWith("#profile") || rawHash.startsWith("#settings") || rawHash.startsWith("#now") || rawHash.startsWith("#stats") || rawHash.startsWith("#universes") || rawHash.startsWith("#dashboard") || rawHash.startsWith("#auth")){
    const [pathPart, queryPart = ""] = rawHash.slice(1).split("?");
    const params = new URLSearchParams(queryPart);
    const segments = pathPart.split("/").filter(Boolean);
    return {
      view: segments[0] || "dashboard",
      category: segments[1] ? decodeURIComponent(segments[1]) : "",
      itemId: segments[2] ? decodeURIComponent(segments[2]) : "",
      filterStatus: params.get("status") || "All",
      filterFolder: params.get("folder") || "All",
...
restoreAppRouteState = async function restoreAppRouteStateStable(){
  if(hasRestoredAppRouteState || isPublicShareRoute()) return false;
  const state = parseAppRouteState();
  hasRestoredAppRouteState = true;
  if(!state?.view) return false;
  ...
  try {
    ...
    window.setTimeout(() => window.scrollTo({ top: Number(state.scrollY || 0), behavior: "auto" }), 0);
    return true;
  } finally {
    isRestoringAppRouteState = false;
    syncShelfSearchInput();
...
updateBottomNavState = function updateBottomNavStateStable(section){
  const map = {
    ...
    profile: "nav-settings-btn",
    category: "nav-library-btn",
    details: "nav-library-btn"
  };
  Object.values(map).forEach((id) => document.getElementById(id)?.classList.remove("is-active"));
  if(map[section]) document.getElementById(map[section])?.classList.add("is-active");
};
...
function buildCollectionsSnapshot(){
  ...
  items.forEach((item) => {
    const folder = normalizeSpaces(item.folder || "") || "__ungrouped__";
    const key = `${item.category}:${folder}`;
  ...
  return {
    collections: Array.from(folderMap.values()).sort((left, right) => String(left.category).localeCompare(String(right.category)) || String(left.folder).localeCompare(String(right.folder))),
    namedCollections: Array.from(folderMap.values()).filter((entry) => entry.folder !== "__ungrouped__").length,
    categoriesWithFolders: byCategory.size,
    totalItems: items.length,
    ungroupedItems: items.filter((item) => !normalizeSpaces(item.folder || "")).length
  };
}

async function openCollectionFromScreen(category, folder){
  currentFilterFolder = folder || "All";
  localStorage.setItem("plamut_folder_filter", currentFilterFolder);
  await openCategory(category, { preserveSearch: true });
  renderFolderRail();
  renderShelf();
}

async function renderCollectionsScreen(){
  const summary = document.getElementById("collections-summary-grid");
  const list = document.getElementById("collections-list");
  if(!summary || !list) return;
  const snapshot = buildCollectionsSnapshot();
  summary.innerHTML = [
    [currentLanguage === "ru" ? "Коллекции" : "Collections", snapshot.namedCollections],
    [currentLanguage === "ru" ? "Категории" : "Categories", snapshot.categoriesWithFolders],
    [t().labels.ungroupedItems || (currentLanguage === "ru" ? "Без папки" : "Ungrouped"), snapshot.ungroupedItems],
    [t().labels.allItems || (currentLanguage === "ru" ? "Все элементы" : "All items"), snapshot.totalItems]
  ].map(([label, value]) => `<article class="stat-card"><div class="stat-card-value">${escapeHtml(String(value))}</div><div class="stat-card-label">${escapeHtml(String(label))}</div></article>`).join("");
  list.innerHTML = snapshot.collections.length ? snapshot.collections.map((entry) => `
    <article class="collection-card">
      <div class="collection-card-header">
        <div>
          <h3 class="collection-card-title">${escapeHtml(entry.folder === "__ungrouped__" ? (t().labels.ungroupedItems || "Без папки") : entry.folder)}</h3>
          <div class="collection-card-meta">${escapeHtml(translateCategory(entry.category))} · ${entry.items.length}</div>
        </div>
        <button class="button button-secondary" type="button" onclick="openCollectionFromScreen('${escapeHtml(entry.category)}', '${escapeHtml(entry.folder)}')">${escapeHtml(t().collections?.open || t().buttons.open)}</button>
      </div>
      <div class="collection-chip-row">${entry.items.slice(0, 5).map((item) => `<span class="collection-chip">${escapeHtml(item.title)}<span class="collection-chip-count">${escapeHtml(translateStatus(item.status || 'Planned'))}</span></span>`).join("")}</div>
    </article>
  `).join("") : `<div class="small">${escapeHtml(t().labels.foldersEmpty || t().labels.noResults)}</div>`;
}

async function renderProfileScreen(){
  const container = document.getElementById("profile-screen-summary");
  if(!container) return;
  const user = await getCurrentUser();
  const profile = await ensureCurrentProfileData().catch(() => currentProfileData || null);
  const title = normalizeSpaces(profile?.display_name || profile?.username || user?.email || "Plamut");
  const subtitle = normalizeSpaces(profile?.username ? `@${profile.username}` : user?.email || "");
  const shareEnabled = profile ? isShareEnabled(profile) : false;
  const routeState = buildAppRouteState();
  container.innerHTML = `
    <article class="settings-card">
      <div class="settings-card-header">
        <div>
          <h3 class="settings-card-title">${escapeHtml(title)}</h3>
          <div class="settings-card-meta">${escapeHtml(subtitle || (currentLanguage === "ru" ? "Личный профиль" : "Private profile"))}</div>
        </div>
      </div>
      <div class="settings-card-meta">${escapeHtml((currentLanguage === "ru" ? "Публичная библиотека" : "Public library") + ': ' + (shareEnabled ? (currentLanguage === "ru" ? "включена" : "enabled") : (currentLanguage === "ru" ? "выключена" : "disabled")))}</div>
      <div class="settings-card-meta">${escapeHtml((t().settingsPage?.currentPage || 'Current page') + ': ' + routeState.view)}</div>
      <div class="settings-card-actions">
        <button class="button button-primary" type="button" onclick="openProfileModal()">${escapeHtml(t().profile.save || (currentLanguage === "ru" ? 'Редактировать профиль' : 'Edit profile'))}</button>
        <button class="button button-secondary" type="button" onclick="openShareModal()">${escapeHtml(t().settingsPage?.sharing || t().topbar.shareLibrary)}</button>
        <button class="button button-secondary" type="button" onclick="openCurrentPublicCard()">${escapeHtml(currentLanguage === "ru" ? 'Открыть публичную карточку' : 'Open public card')}</button>
      </div>
    </article>
  `;
}
...
async function renderSettingsScreen(){
  const container = document.getElementById("settings-stack");
  if(!container) return;
  const user = await getCurrentUser();
  const profile = await ensureCurrentProfileData().catch(() => currentProfileData || null);
  const sessionState = currentSupabaseSession?.user ? (currentLanguage === "ru" ? "Активна" : "Active") : (currentLanguage === "ru" ? "Нет сессии" : "No session");
  const shareUrl = getCurrentShareUrl();
  container.innerHTML = `
    <article class="settings-card">
      <div class="settings-card-header">
        <div>
          <h3 class="settings-card-title">${escapeHtml(t().settingsPage?.session || 'Session')}</h3>
          <div class="settings-card-meta">${escapeHtml(user?.email || (currentLanguage === "ru" ? 'Пользователь не найден' : 'User not found'))}</div>
        </div>
      </div>
      <div class="settings-card-meta">${escapeHtml(sessionState)} · ${escapeHtml(t().settingsPage?.restore || 'Restored automatically after refresh')}</div>
      <div class="settings-card-actions">
        <button class="button button-secondary" type="button" onclick="showProfileScreen()">${escapeHtml(t().settingsPage?.profile || t().topbar.profile)}</button>
        <button class="button button-ghost" type="button" onclick="logout()">${escapeHtml(t().topbar.logout)}</button>
      </div>
    </article>
    <article class="settings-card">
      <div class="settings-card-header">
        <div>
          <h3 class="settings-card-title">${escapeHtml(t().settingsPage?.interface || t().topbar.interface)}</h3>
          <div class="settings-card-meta">${escapeHtml(`${t().topbar.language}: ${getLanguageLabel()} · ${t().topbar.theme}: ${getThemeModeLabel()}`)}</div>
        </div>
      </div>
      <div class="settings-card-actions">
        <button class="button button-secondary" type="button" onclick="setLanguage(currentLanguage === 'ru' ? 'en' : 'ru')">${escapeHtml(currentLanguage === 'ru' ? 'Switch to EN' : 'Переключить на RU')}</button>
        <button class="button button-secondary" type="button" onclick="setThemeMode(currentThemeMode === 'dark' ? 'light' : 'dark')">${escapeHtml(currentLanguage === 'ru' ? 'Сменить тему' : 'Toggle theme')}</button>
      </div>
    </article>
    <article class="settings-card">
      <div class="settings-card-header">
        <div>
          <h3 class="settings-card-title">${escapeHtml(t().settingsPage?.sharing || t().topbar.shareLibrary)}</h3>
          <div class="settings-card-meta">${escapeHtml(isShareEnabled(profile || {}) ? shareUrl || (currentLanguage === 'ru' ? 'Ссылка готова' : 'Link ready') : (currentLanguage === 'ru' ? 'Отключено' : 'Disabled'))}</div>
        </div>
      </div>
      <div class="settings-card-actions">
...
function showCollectionsScreen(){
  ...
  updateBottomNavState("collections");
...
requestProductRefresh = function requestProductRefreshStable(options = {}){
  productRefreshNeedsFull = productRefreshNeedsFull || Boolean(options.forceFull);
  if(productRefreshScheduled) return;
  productRefreshScheduled = true;
  requestAnimationFrame(async () => {
    ...
    try {
      ...
      if(forceFull || view === "library") await renderLibraryRootScreen();
      if(forceFull || view === "collections") await renderCollectionsScreen();
      if(forceFull || view === "profile") await renderProfileScreen();
      if(forceFull || view === "settings") await renderSettingsScreen();
      if(forceFull || view === "now") await renderNowScreen();
      if(forceFull || view === "stats") await Promise.all([renderStatsScreen(), renderActivityLists()]);
      if(forceFull || view === "universes") await renderUniversesScreen();
      if(forceFull || view === "details" || options.detailsOnly){
        await Promise.all([renderTrackerSummaryForCurrentItem(), renderUniverseDetailsForCurrentItem()]);
      }
    } catch (error) {
...
openProfileFromMenu = function openProfileFromMenuStable(){
  closeProfileMenu();
  showProfileScreen();
};

checkAuth = async function checkAuthStable(){
  const session = await bootstrapSupabaseSession();
...
showAuthorizedUI = async function showAuthorizedUIStable(){
  closePreferencesPanel();
  setPublicRouteMode(false);
  await hydrateLibraryFromSupabase({ force: false });
  hideAllScreens();
  document.getElementById("home-screen")?.classList.remove("hidden");
  setAuthorizedButtons(true);
  refreshAccountCollectionsUI();
...
showAuthScreen = function showAuthScreenStable(){
  closePreferencesPanel();
  hideAllScreens();
  document.getElementById("auth-screen")?.classList.remove("hidden");
...
login = async function loginStable(){
  const email = document.getElementById("login-email")?.value.trim();
  const password = document.getElementById("login-password")?.value || "";
...
logout = async function logoutStable(){
  closePreferencesPanel();
  closeProfileModal();
  closeShareModal();
  closeShareSheet();
  ...
  localStorage.removeItem("plamut_app_route_state");
  invalidateLibraryHydration();
  showAuthScreen();
  setAuthorizedButtons(false);
};

initApp = async function initAppStable(){
  setPublicRouteMode(false);
  if(!window.__plamutAuthListenerBound){
    window.__plamutAuthListenerBound = true;
    supabaseClient.auth.onAuthStateChange(async (_event, session) => {
...
const baseSetLanguageStable = setLanguage;
setLanguage = function setLanguageStable(lang){
  baseSetLanguageStable(lang);
  requestProductRefresh({ forceFull: true });
};

const baseSetThemeModeStable = setThemeMode;
setThemeMode = function setThemeModeStable(mode){
  baseSetThemeModeStable(mode);
  requestProductRefresh({ forceFull: false });
};

const previousApplyTranslationsStable = applyTranslations;
applyTranslations = function applyTranslationsStable(){
  previousApplyTranslationsStable();
  [
    ["collections-screen-title", currentLanguage === "ru" ? "Коллекции" : "Collections"],
    ["collections-section-title", currentLanguage === "ru" ? "Папки и коллекции" : "Folders & collections"],
    ["collections-section-note", currentLanguage === "ru" ? "Все ваши папки по категориям." : "All folders grouped by category."],
    ["profile-screen-page-title", t().topbar.profile],
    ["profile-screen-heading", t().topbar.profile],
    ["profile-screen-note", currentLanguage === "ru" ? "Аккаунт, публичная карточка и быстрые действия." : "Account, public card and quick actions."],
    ...
    ["back-settings-btn", "←"],
    ["home-add-btn", t().buttons.addPrimary],
    ["share-library-btn", t().topbar.shareLibrary]
  ].forEach(([id, value]) => setTextIfPresent(id, value));
  renderCollectionsScreen();
  renderProfileScreen();
  renderSettingsScreen();
...
const baseInitStable = init;
init = async function initStable(){
  applyThemeMode();
  await updateAllProductData();
  applyTranslations();
  updateHeaderCompactState();
  ...
  window.addEventListener("beforeunload", persistAppRouteState);
  window.addEventListener("resize", () => {
    if(!isMobileViewport()) closeItemActionsSheet();
  });
  document.addEventListener("click", (event) => {
  ...
  await initApp();
  updateBottomNavState(getActiveProductView());
  requestProductRefresh({ forceFull: true });
  updatePrimaryActionVisibility();
};
